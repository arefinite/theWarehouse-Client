...
const StockDet = () => {
    ...
    const handlePdUpdate = e => {
        ...
        let updateStocks = parseInt(stockInput?.current?.value)
        let stock
        if(e){
            if(quantity > 0 ){
            ...
            if(!isNaN(updateStock)){
                ...
                setAvailable(false)
                return toast.error("Update Stock Can't Be Empty")
        ...
        axiosPrivate.put(`https://warehouse-api-server.herokuapp.com/stock/${productId}`, {delivery, userInfo} )
        ...
        if(e){
            ...
            toast.success(updateStocks < 2 ? `${updateStocks} Product Added Successfully` : `${updateStocks} Products Added Successfully`)
    ...
    return (
        <div>
            ...
            </Helmet>
            {
                ...
                <div className='hidden md:block md:border-[1px] h-96'></div>
                <div>
                ...
                <div className='flex  items-center ml-8 md:ml-0'>
                    <div>
                        ...
                        <div className='flex justify-center'>
                            <button onClick={()=>handlePdUpdate(false)} className='p-2 mr-5 w-1/2 bg-green-400  font-semibold rounded-lg hover:bg-green-700 hover:text-white duration-300 mt-3'>Update stock</button>
                            <button onClick={()=>handlePdUpdate(true)} disabled={available} className={available? ' disabled:opacity-50 w-1/2 p-2 bg-blue-400 cursor-not-allowed font-semibold rounded-lg mt-3' : 'p-2 bg-blue-400 w-1/2  font-semibold rounded-lg hover:bg-blue-700 hover:text-white duration-300 mt-3'}>Delivery</button>
                        </div>
                    </div>
                </div>
                <button onClick={()=>deleteProduct(_id)} className='p-2 w-full mr-5 bg-red-400  font-semibold rounded-lg hover:bg-red-700 hover:text-white duration-300 mt-3'>Delete Item</button>
                </div>
            </div>
