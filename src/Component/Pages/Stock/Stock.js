...
const Stock = () => {
    ...
    useEffect(() =>{
        axios.get(`https://warehouse-api-server.herokuapp.com/pagesPd?page=${page}&size=${size}`)
        .then(data =>setProducts(data.data))
    },[page, size])
    useEffect(() => {
        axios.get('https://warehouse-api-server.herokuapp.com/allPdCount')
        .then(data => {
    ...
    return (
        <div className="bg-white">
            ...
            <div className='pb-8'>
                <Link to="/add-item" className="text-base bg-red-400 rounded p-2 hover:bg-red-700 hover:text-white font-bold text-white"> Add Items </Link>
                <Link to="/delete-item" className="text-base ml-5 font-bold bg-red-400 rounded p-2 hover:bg-red-700 hover:text-white text-white"> Manege Items </Link>
            </div>
                <h2 className="text-2xl font-semibold tracking-tight text-gray-700">Your Available Stock Product</h2>
                    ...
                    {products.map((product) => (
                        <div key={product._id} className="group relative">
                            <div className=" min-h-80 flex justify-center aspect-w-1 aspect-h-1 rounded-md overflow-hidden group-hover:opacity-75 lg:h-80 lg:aspect-none">
                                ...
                                src={product.img}
                                alt=''
                                className="w-2/3 object-center object-cover rounded-lg border-2 lg:w-full lg:h-full"
                                />
                            </div>
                            <div className="mt-4">
                                <div className='flex'>
